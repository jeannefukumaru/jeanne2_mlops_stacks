resource "databricks_group" "mlops-service-principal-group-staging" {
  display_name = "jeanne2_mlops_stacks-service-principals"
...
module "azure_create_sp" {
  ...
  service_principal_name       = "jeanne2_mlops_stacks-cicd"
  project_directory_path       = "/jeanne2_mlops_stacks"
  azure_tenant_id              = var.azure_tenant_id
  service_principal_group_name = "jeanne2_mlops_stacks-service-principals"
}
...
provider "databricks" {
  alias = "staging_sp"
  host  = "https://adb-2236449858363810.10.azuredatabricks.net"
  token = module.azure_create_sp.staging_service_principal_aad_token
}

provider "databricks" {
  alias = "prod_sp"
  host  = "https://adb-2920824224628045.5.azuredatabricks.net"
